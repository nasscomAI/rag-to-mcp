...
- **Clarification:** If the instructions for a Use Case (UC) are confusing.
...
2. Give it a clear title (e.g., `[UC-RAG] Confusion about chunking strategy`).
...
```bash
git clone https://github.com/[your-username]/RAG-to-MCP.git
cd RAG-to-MCP
```
...
```bash
git add .
git commit -m "[UC-RAG] Fix chunk boundary: fixed-size split → sentence-aware chunking"
git push origin participant/[your-name]-[city]
```

### Step 5: Open the PR
1. Go to the **original** repository (nasscomAI/RAG-to-MCP).
...
1. **Gemini CLI:** A powerful AI-native terminal agent that can analyze code, run tests, and manage your entire git workflow.
2. **Claude Code:** Anthropic's terminal-based coding agent for complex projects.
3. **Aider:** A popular CLI tool that lets you edit code in your terminal using AI. (Requires an API key).
4. **Mentat:** An open-source AI coding assistant that lives in your terminal and can understand your entire codebase.
...
## RAG/MCP-Specific Troubleshooting

### ChromaDB "Collection already exists" error
If you see `chromadb.errors.DuplicateCollectionError`:
- Delete the `uc-rag/chroma_db/` directory and rebuild the index
- Or use `client.get_or_create_collection()` instead of `client.create_collection()`

### sentence-transformers download timeout
The `all-MiniLM-L6-v2` model is ~80MB. If the download times out:
- Run the install from a stable connection the night before: `python3 -c "from sentence_transformers import SentenceTransformer; SentenceTransformer('all-MiniLM-L6-v2')"`
- See `docs/pre-session-install.md` for the full offline setup

### GEMINI_API_KEY not set
If you see `google.api_core.exceptions.PermissionDenied` or `API key not valid`:
- Verify: `echo $GEMINI_API_KEY` (should print your key)
- Set it: `export GEMINI_API_KEY="your-key-here"`
- On Windows: `set GEMINI_API_KEY=your-key-here`
- Alternatives: set `ANTHROPIC_API_KEY` or `OPENAI_API_KEY` and update `llm_adapter.py`

### stub_rag.py index not built before UC-MCP
UC-MCP depends on a running RAG server. If you haven't built UC-RAG yet:
- Run `python3 uc-rag/stub_rag.py --build-index` to create the fallback index
- Then start the stub: `python3 uc-rag/stub_rag.py`

### test_client.py "Connection refused"
The MCP test client expects a running server on localhost:
- Start your MCP server first: `python3 uc-mcp/mcp_server.py`
- Then in a separate terminal: `python3 uc-mcp/test_client.py --run-all`
...
1. **Commit Message:** Ensure your message follows the exact `[UC-ID] Fix... → ...` format. Valid prefixes: `UC-0A`, `UC-RAG`, `UC-MCP`.
